            ...
            while (true)
            {
               ...
               if (coinControl && !coinControl->fSplitBlock)
               {
                  for (const PAIRTYPE(CScript, CAmount)& s : vecSend)
                  {
                     ...
                     if (txout.IsDust(::minRelayTxFee) && !txout.scriptPubKey.HasOpCreate() &&
               ...
               if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false,
                                fIncludeDelegated, fIncludeLeased))
               {
                  ...
                  {
                     strFailReason = _(
                     "Unable to locate enough funds for this transaction that are not equal 1000 BTCU.");
                  }
                  else if (coin_type == ONLY_NONDENOMINATED_NOT1000IFMN)
                  {
                     strFailReason = _(
                     "Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 1000 BTCU.");
                  }
                  else
                  {
...
bool CWallet::SetAddressBook(const CWDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(Standard::EncodeDestination(address), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(Standard::EncodeDestination(address), strName);
}
...
libzcash::SaplingPaymentAddress CWallet::GenerateNewSaplingZKey(std::string label) {
    ...
    SetAddressBook(address, label, AddressBook::AddressBookPurpose::SEND);
    return address;
}
