...
bool CBech32Data::SetString(const std::string& str)
{
    ...
    if (bech.data.empty() || bech.hrp != Params().Bech32HRP(CChainParams::SAPLING_PAYMENT_ADDRESS)) {
        return false;
    }

    int nTmpVersion = bech.data[0]; // The first 5 bit symbol is the witness version (0-16)
    std::vector<unsigned char> vchTemp;
    vchTemp.reserve(((bech.data.size() - 1) * 5) / 8);

    if (!ConvertBits<5, 8, false>([&](unsigned char c) { vchTemp.push_back(c); }, bech.data.begin() + 1, bech.data.end())) {
        return false;
    }
...
{
    return bech32::Encode(bech32::Encoding::BECH32, Params().Bech32HRP(CChainParams::SAPLING_FULL_VIEWING_KEY), vchData);
}
