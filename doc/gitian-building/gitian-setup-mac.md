...
The build script we'll be using is contained in the BTCU github repository ([contrib/gitian-build.py](https://github.com/btcu-project/btcu/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the BTCU repository and will need to fetch this script with the following commands:
